...
/*   Updated: 2025/03/04 00:31:53 by mbaypara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	sphere_intersection(t_ray ray, t_sphere sphere, t_impact *impact)
{
	...
	// ray ile kurenin merkezi arasindaki fark
	...
	else if (a <= 0.) // gecerli degilse
		a = b;
	else if (b <= 0.) // camera onunde olan noktayi aliyoruz
		b = a;
	impact->distance = get_minf(a, b);  // eger ikisi de poziitfse kameraya en yakin olan noktayi aliyoruz
	vector = vec3_add(ray.origin, vec3_mult(ray.dir, b)); // isin yonu mesafe ile carpiliyor rayin baslangic noktasina ekleniyor
	impact->normal = vec3_norm(vec3_sub(vector, sphere.pos)); // kesisim noktasi ile kure merkezi arasinin farki normalizasyonu yapiliyor isik icin
...
void	sphere_ray(t_ray ray, t_scene *scene, t_impact *impact, void **obj)
{
	...
	while (lst->next)
	{
		...
		{
			*obj = sphere; // carpisan objeyi obje pointerina atiyoruz
			impact->object = "sp"; // isim verdik
		}
		lst = lst->next;
	}
}
